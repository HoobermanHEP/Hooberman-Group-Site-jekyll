...
All Hooberman HEP group members are supposed to know how to do coding. So I'll put it directly here. After we make changes to the code, the compilation process is also required in order to see the changes in the software. Jekyll is the same case. Although html is equal to the plain text plus descriptive tags, there are lots of sub-parts of the website are used lots of times. Without Jekyll, you have to copy/paste each time you create a page. With Jekyll, you can just use layouts and variables with the help of Liquid language and Jekyll will turn all of these to the webpage you wish to have. In other words, Jekyll reduces the time cost and complexity.
...
4. Run Jekyll (by command jekyll build).
